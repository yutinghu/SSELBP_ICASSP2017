...
%         j = bitset(bitshift(i,1,samples),1,bitget(i,samples));
        j = bitset(bitshift(i,1,ASSUMEDTYPE),1,bitget(i,samples));
...
%             r = bitset(bitshift(r,1,samples),1,bitget(r,samples)); % rotate left
            r = bitset(bitshift(r,1,ASSUMEDTYPE),1,bitget(r,samples)); % rotate left
            if r < rm
                rm = r;
            end
        end
...
%         j = bitset(bitshift(i,1,samples),1,bitget(i,samples)); % rotate left
        j = bitset(bitshift(i,1,ASSUMEDTYPE),1,bitget(i,samples)); % rotate left
        numt = sum(bitget(bitxor(i,j),1:samples));
        if numt <= 2
